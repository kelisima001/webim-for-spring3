...
function(p,v){j.trigger("update",[p,v])});l=K({},q,l);j.data=l;T({type:"post",url:G("setting"),cache:false,data:{data:na.stringify(l),csrf_token:J.csrf_token}})}}}});ka("status",{key:"_webim",storage:"local",domain:D.domain},{_init:function(){var e=this.data,g=this.options.key,j=this.options.storage=="local"&&y.localStorage;if(j)try{j.setItem("__store_webim__","__store_webim__");if(j.getItem("__store_webim__")=="__store_webim__")this.store=j;j.removeItem("__store_webim__")}catch(l){this.store=ja}if(e)this._save(e);
else this.data=(e=this.store?this.store.getItem(g):ra(g))?na.parse(e):{}},set:function(e,g){var j=e;if(typeof e=="string"){j={};j[e]=g}var l=this.data;pa(l,j)&&this._save(K({},l,j))},get:function(e){return this.data[e]},clear:function(){this._save({})},_save:function(e){var g=this.options.key,j=this.options.domain;this.data=e;e=na.stringify(e);this.store?this.store.setItem(g,e):ra(g,e,{path:"/",domain:j})}});ka("buddy",{active:true},{_init:function(){this.data=this.data||[];this.dataHash={};this.set(this.data)},
remove:function(e){var g=this.get(e);if(g){T({type:"post",url:G("remove_buddy"),cache:false,data:{id:e,csrf_token:J.csrf_token},success:function(){}});this.trigger("unsubscribe",[[g]]);delete this.dataHash[e]}},clear:function(){this.data=[];this.dataHash={}},count:function(e){var g=this.dataHash,j=0,l;for(var q in g)if(Ca(e)){l=true;for(var r in e)if(e[r]!=g[q][r])l=false;l&&j++}else j++;return j},get:function(e){return this.dataHash[e]},all:function(e){return e?fa(this.data,function(g){return g.show!=
"invisible"&&g.presence=="online"}):this.data},complete:function(){var e=this.dataHash,g=[],j;for(var l in e){j=e[l];if(j.incomplete){j.incomplete=false;g.push(l)}}this.load(g)},update:function(e){this.load(e)},presence:function(e){var g=this.dataHash;e=za(e)?e:[e];for(var j in e){var l=e[j];l.presence=l.presence=="offline"?"offline":"online";l.incomplete=!g[l.id];if(!l.group&&l.id)l.group=l.id.indexOf("vid:")==0?"visitor":l.group}this.set(e)},load:function(e){e=x(e);e.length&&T({type:"get",url:G("buddies"),
cache:false,data:{ids:e.join(","),csrf_token:J.csrf_token},context:this,success:this.set})},set:function(e){var g=this.data,j=this.dataHash,l={};e=e||[];for(var q=e.length,r,p,v,F=0;F<q;F++){r=e[F];if(v=r.id){if(!j[v]){r.presence=r.presence||"online";r.show=r.show?r.show:r.presence=="offline"?"unavailable":"available";j[v]={};g.push(j[v])}r.incomplete=!!r.incomplete;if(p=pa(j[v],r)){r=p.presence||"update";l[r]=l[r]||[];K(j[v],p);l[r].push(j[v])}}}for(var H in l)this.trigger(H,[l[H]]);this.options.active&&
this.complete()}});ka("presence",{},{_init:function(){this.data=this.data||{};this.set(this.data)},get:function(e){return this.data[e]},set:function(e){var g={};for(var j in e){var l=e[j],q="online";if(l=="unavailable"||l=="invisible")q="offline";g[q]=g[q]||[];g[q].push({id:j,show:l})}for(var r in g)this.trigger(r,[g[r]])},update:function(e){for(var g={},j=0;j<e.length;j++){var l=e[j];g[l.from]=l.show}this.set(g)},clear:function(){this.data=[]}});(function(){ka("room",{},{_init:function(){this.data=
this.data||[];this.dataHash={}},get:function(e){return this.dataHash[e]},all:function(e){return e?fa(this.data,function(g){return g.temporary}):this.data},invite:function(e,g,j,l){var q=this,r=q.options;T({type:"post",cache:false,url:G("invite"),data:{ticket:r.ticket,id:e,nick:g||"",members:j.join(","),csrf_token:J.csrf_token},success:function(p){q.set([p]);q.loadMember(e);l&&l(p)}})},join:function(e,g,j){var l=this,q=l.options;T({type:"post",cache:false,url:G("join"),data:{ticket:q.ticket,id:e,nick:g||
"",csrf_token:J.csrf_token},success:function(r){l.set([r]);l.loadMember(e);j&&j(r)}})},leave:function(e){var g=this,j=g.options,l=g.dataHash[e],q=j.user;l&&T({type:"post",cache:false,url:G("leave"),data:{ticket:j.ticket,id:e,nick:q.nick,temporary:l.temporary,csrf_token:J.csrf_token},success:function(){delete g.dataHash[e];g.trigger("leaved",[e])}})},block:function(e){var g=this,j=g.options,l=g.dataHash[e];if(l&&!l.blocked){l.blocked=true;var q=[];O(g.dataHash,function(r,p){!p.temporary&&p.blocked&&
q.push(p.id)});T({type:"post",cache:false,url:G("block"),data:{ticket:j.ticket,id:e,csrf_token:J.csrf_token},success:function(){g.trigger("blocked",[e,q])}})}},unblock:function(e){var g=this,j=g.options,l=g.dataHash[e];if(l&&l.blocked){l.blocked=false;var q=[];O(g.dataHash,function(r,p){!p.temporary&&p.blocked&&q.push(p.id)});T({type:"post",cache:false,url:G("unblock"),data:{ticket:j.ticket,id:e,csrf_token:J.csrf_token},success:function(){g.trigger("unblocked",[e,q])}})}},set:function(e){var g=this,
j=g.data,l=g.dataHash;O(e,function(q,r){var p=r.id;if(p){r.members=r.members||[];r.all_count=r.members.length;r.count=0;O(r.members,function(v,F){if(F.presence=="online")r.count+=1});if(l[p])K(l[p],r);else{l[p]=r;j.push(r)}g.trigger("updated",l[p])}})},loadMember:function(e){var g=this,j=g.options;T({type:"get",cache:false,url:G("members"),data:{ticket:j.ticket,id:e,csrf_token:J.csrf_token},success:function(l){g.updateMember(e,l)}})},updateMember:function(e,g){var j=this.dataHash[e];if(j){j.memberLoaded=
true;j.members=g;this.set([j])}},onPresence:function(e){var g=e.type;if(e.to&&this.dataHash[e.to]){var j=e.to,l=this.dataHash[j];l&&l.memberLoaded&&this.loadMember(j);if(g=="join")this.trigger("memberJoined",[j,e]);else if(g=="leave")this.trigger("memberLeaved",[j,e]);else if(g=="grponline")this.trigger("memberOnline",[j,e]);else g=="grpoffline"&&this.trigger("memberOffline",[j,e])}},clear:function(){this.data=[];this.dataHash={}}})})();ka("history",{},{_init:function(){this.data=this.data||{};this.data.chat=
this.data.chat||{};this.data.grpchat=this.data.grpchat||{}},clean:function(){this.data.chat={};this.data.grpchat={}},get:function(e,g){return this.data[e][g]},set:function(e){var g=this.data,j={chat:{},grpchat:{}};e=la(e);var l=e.length,q,r,p=this.options.userInfo.id;if(l){for(var v=0;v<l;v++){q=e[v];F=q.type;if((r=F=="chat"?q.to==p?q.from:q.to:q.to)&&F){j[F][r]=j[F][r]||[];j[F][r].push(q)}}for(var F in j)for(r in j[F]){q=j[F][r];if(g[F][r]){g[F][r]=[].concat(g[F][r]).concat(q);this._triggerMsg(F,
r,q)}else this.load(F,r)}}},_triggerMsg:function(e,g,j){this.trigger(e,[g,j])},clear:function(e,g){this.data[e][g]=[];this.trigger("clear",[e,g]);T({url:G("clear"),type:"post",cache:false,data:{type:e,id:g,csrf_token:J.csrf_token}})},download:function(e,g){var j=G("download"),l=D.createElement("iframe"),q=new Date,r=[];q={id:g,type:e,time:(new Date).getTime(),date:q.getFullYear()+"-"+(q.getMonth()+1)+"-"+q.getDate()};for(var p in q)r[r.length]=encodeURIComponent(p)+"="+encodeURIComponent(q[p]);j+=
(/\?/.test(j)?"&":"?")+r.join("&");l.setAttribute("src",j);l.style.display="none";D.body.appendChild(l)},init:function(e,g,j){if(za(j)){this.data[e][g]=j;this._triggerMsg(e,g,j)}},load:function(e,g){var j=this;j.data[e][g]=[];T({url:G("history"),cache:false,type:"get",data:{type:e,id:g,csrf_token:J.csrf_token},success:function(l){j.init(e,g,l)}})}})})(window,document);
